...
partial class RefitBodyBuilder : IRequestBuilder
{
    static readonly QueryAttribute DefaultQueryAttribute = new ();
    static readonly Uri BaseUri = new ("http://api");
    readonly Dictionary<string, List<RestMethodSymbolInternal>> interfaceHttpMethods;
    ...
    )
    {
        ...
        interfaceGenericHttpMethods =
            new ConcurrentDictionary<CloseGenericMethodKey, RestMethodSymbolInternal>();
        ...
        var dict = new Dictionary<string, List<RestMethodSymbolInternal>>();
    ...
    void AddInterfaceHttpMethods(
        Type interfaceType,
        Dictionary<string, List<RestMethodSymbolInternal>> methods
    )
    {
        ...
        foreach (var methodInfo in methodInfos)
        {
            ...
            if (hasHttpMethod)
            {
                ...
                var restinfo = new RestMethodSymbolInternal(interfaceType, methodInfo, settings);
                value.Add(restinfo);
            }
        }
    }

    // RestMethodSymbolInternal FindMatchingRestMethodInfo(
    //     string key,
    //     Type[]? parameterTypes,
    ...
    RestMethodSymbolInternal CloseGenericMethodIfNeeded(
        RestMethodSymbolInternal restMethodInfo,
        Type[]? genericArgumentTypes
    )
    {
    ...
    Func<HttpClient, CancellationToken, object[], Task<T?>> BuildCancellableTaskFuncForMethod<
        T,
        TBody
    >(RestMethodSymbolInternal restMethod)
    {
    ...
    )
    {
        ...
        var paramList = restMethod.ParameterSymbolArray;
    ...
    string AddValueParameterToUrl(RestMethodSymbolInternal restMethod, RestMethodParameterInfo parameterMapValue,
    ...
    void AddBodyToRequest(RestMethodSymbolInternal restMethod, object param, HttpRequestMessage ret)
    {
...
#pragma warning restore IDE1006 // Naming Styles
    ...
    void AddQueryParameters(RestMethodSymbolInternal restMethod, QueryAttribute? queryAttribute, object param,
    ...
    void AddMultiPart(RestMethodSymbolInternal restMethod, int i, object param,
        MultipartFormDataContent? multiPartContent)
    {
    ...
    void AddPropertiesToRequest(RestMethodSymbolInternal restMethod, HttpRequestMessage ret, object[] paramList)
    {
...
#pragma warning disable CA2000
        var vsb = new Refit.ValueStringBuilder(stackalloc char[512]);
